...
const GLOSSARY: Record<string, { title: string; text: string }> = {
  ...
  cacheWrite: { title: 'Cache Write', text: 'Tokens written into the prompt cache on this request. Billed once, then reused as Cache Read on later turns. Some models bill cache writes without reporting them — for those, the uncached part of the prompt is counted as the cache write.' },
...
/** True for turns Copilot generated itself (terminal follow-ups, subagents). */
function isAutoTurn(t: TurnCost): boolean {
  return t.origin === 'auto-terminal' || t.origin === 'subagent';
}

const AUTO_BADGE = '<span class="origin-badge" title="Generated by Copilot, not typed by you">auto</span> ';

/** Prompt label cell content: badge + readable label, or the muted placeholder. */
function promptLabelHtml(t: TurnCost): string {
  const text = t.displayLabel || t.label;
  if (!text) return '<span class="prompts-muted">(no prompt text)</span>';
  return (isAutoTurn(t) ? AUTO_BADGE : '') + escapeHtml(text);
}

/** Raw prompt text for the row tooltip, so the original message stays reachable. */
function promptTooltip(t: TurnCost): string {
  return t.label || t.displayLabel || '';
}

function promptCountLabel(count: number, autoCount: number): string {
  const base = count + ' prompt' + (count === 1 ? '' : 's');
  return autoCount > 0 ? base + ' · ' + autoCount + ' auto' : base;
}
...
function sessionAggregate(g: SessionGroup): { count: number; autoCount: number; reqs: number; cost: number; input: number; output: number; cached: number } {
  const autoCount = g.items.filter(({ turn }) => isAutoTurn(turn)).length;
  const count = g.items.length - autoCount;
  if (g.info) {
    return { count, autoCount, reqs: g.info.modelTurns, cost: g.info.totalCost, input: g.info.inputTokens, output: g.info.outputTokens, cached: g.info.cachedTokens };
  }
...
function renderPromptRow(t: RecentPrompt, sessionId: string): string {
  return '<tr class="prompt-row prompt-subrow" data-session-id="' + escapeHtml(sessionId) + '" data-trace-id="' + escapeHtml(t.traceId) + '" title="Click to see all interactions">' +
    ...
    '<td class="prompts-label prompt-indent" title="' + escapeHtml(promptTooltip(t)) + '">' + promptLabelHtml(t) + '</td>' +
...
function renderSessionGroup(g: SessionGroup): string {
  ...
  const countLabel = loaded
    ? '<span class="prompts-muted">' + promptCountLabel(agg.count, agg.autoCount) + '</span>'
    : '';
...
function renderModalBody(turn: RecentPrompt): void {
  ...
  if (!body) return;
  if (title) title.textContent = turn.displayLabel || turn.label || turn.sessionTitle || 'Prompt detail';
  ...
  for (const { turn } of group.items) {
    ...
    html += '<div class="session-prompt">' +
      '<div class="session-prompt-head" data-session-turn="' + escapeHtml(turn.traceId) + '">' +
        '<span class="section-chevron">' + (isCol ? '▸' : '▾') + '</span>' +
        '<span class="session-prompt-label" title="' + escapeHtml(promptTooltip(turn)) + '">' + promptLabelHtml(turn) + '</span>' +
