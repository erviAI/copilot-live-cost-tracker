  ...
  <script nonce="${nonce}">
    ...
    document.addEventListener('click', function(e) {
      ...
      if (promptRow && promptRow.dataset.sessionId) {
        vscode.postMessage({
          command: 'openSessionModal',
          sessionId: promptRow.dataset.sessionId,
          traceId: promptRow.dataset.traceId || ''
        });
        return;
      }
    ...
    function renderSessionDetailInline(sessionId, data) {
      ...
      if (data.turns && data.turns.length > 0) {
        ...
        var autoTurns = data.turns.filter(isAutoTurn).length;
        var userTurns = data.turns.length - autoTurns;
        var turnsCount = userTurns + ' prompts' + (autoTurns > 0 ? ' + ' + autoTurns + ' auto' : '');
        html += '<div class="detail-section"><div class="detail-section-title turns-section-title" data-wrapper="' + turnsWrapperId + '"><span class="section-chevron' + (turnsOpen ? ' open' : '') + '">&#9654;</span> Cost per User Prompt (' + turnsCount + ')</div>';
        ...
        data.turns.forEach(function(t, idx) {
          ...
          var turnLabel = escapeHtml(t.displayLabel || t.label || '');
          var autoBadge = isAutoTurn(t) ? '<span class="origin-badge" title="Generated by Copilot, not typed by you">auto</span> ' : '';
          var totalTokens = (t.inputTokens || 0) + (t.outputTokens || 0) + (t.cachedTokens || 0);
          var tipLines = [
            isAutoTurn(t) ? 'Origin: automatic follow-up (not a user prompt)' : '',
          ...
          html += '<tr class="turn-row prompt-open has-tip" data-session-id="' + escapeHtml(sessionId) + '" data-trace-id="' + escapeHtml(t.traceId || '') + '" title="Open full detail in dashboard">' +
            '<td>' + (timeLabel ? '<span class="turn-time">[' + timeLabel + ']</span> ' : '') + autoBadge + (turnLabel ? '<span class="turn-label">' + turnLabel + '</span>' : (agentLabel ? '<span class="turn-agent">' + agentLabel + '</span> ' : '') + '<span class="turn-trace">' + traceLabel + '</span>') +
