...
description: 'Detailed report on a GitHub Copilot Chat session by joining `agent-traces.db` (spans/tokens/tools) with `session-store.db` (turns/files/checkpoints). Use when: drilling into a single session, comparing model usage, listing tracked files, viewing user messages, debugging tool errors. If no session id is provided, lists the 5 most recent sessions.'
...
If the user provides no session id, the skill shows the 5 most recent sessions (id, model, duration, llm/tool calls, tokens) so they can pick one.
...
- `agent-traces.db` uses WAL; pending writes in `*.db-wal` are invisible to `sql.js`. Close VS Code Chat or use a WAL-aware sqlite client for fresh data.
...
Requires `sql.js` (already installed in `tools/agent-traces/node_modules`). To reinstall:

```powershell
pushd tools/agent-traces; npm install sql.js; popd
```
